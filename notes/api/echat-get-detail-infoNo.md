...
> ⚠️ **同支 API 另一份文件（PDF「信件即時通整併-取得指定記訊明細」，UPDATE 2026/6/29）對 `interViewKind` 的欄位用意標註為 `0詢問意願 1現場面試 2遠距 3刪除`**（無「前端備註／前端判斷」欄），與本表 `3:取消面試` 的標籤不同（`刪除` vs `取消面試`）。本 repo 判斷：本表含前端判斷欄位、且與 §訊息樣式／§跨系統流程 已交叉驗證一致（`type8&&interViewKind3→面試取消`），視為權威版本；PDF 版本的 `3:刪除` 可能是舊標籤或欄位語意誤植，待與後端確認後修正本註記。
