...
需要把 Excel/CSV/MD 分析結果**寫成報告**（MD 或可離線開啟的靜態 HTML，非規格書）時，用 `Skill` 工具載入 `.claude/skills/report-generator/SKILL.md`（§A：MD 結論先行/表格瘦身寫作鐵則；§B：HTML 流程——pandas 聚合 → 讀官方 `dataviz` skill 畫圖 → 模板注入 → Playwright 雙模截圖驗證 → 可重跑 pipeline）。
