...
# E.1 聊天室面試行程確認區塊（§1.3.6）截圖慣例

來源：Figma `s6nrvHrkHpuAPvNZPrYG9D` 節點 `7808:49945`（面試各個狀態）。

**觸發**：廠商發送面試邀約 → 求職者於求職主網回覆「有意願」並同意時間 → 求職後端寫入雙方面試行事曆 → signalR 即時推送 → 聊天室頂部顯示橘色固定橫欄。不隨訊息捲動。

**五個 Figma 狀態**（節點 ID / 說明 / 是否顯示橘色區塊）：

| 節點 | 狀態 | 橘色區塊 |
| --- | --- | :---: |
| `7808:49946` | 詢問意願泡泡 | ✗ |
| `7808:50308` | 面試邀約泡泡（等待回覆）| ✗ |
| `7808:50696` | 求職者同意面試 | ✓ 展開 |
| `7808:51076` | 求職者婉拒面試 | ✗ |
| `7808:51844` | 同意面試＋更多下拉選單 | ✓ 下拉 |

**截圖規格（共兩張）**：
* **展開狀態**（`7808:50696`）：`E1_interview_confirmed.png` — 裁切至橘色橫欄＋展開資訊（回覆期限／聯絡人／電話）＋操作按鈕（取消面試／改時間），不含周圍聊天背景。標注徽章：`面試摘要列`、`展開資訊`、`操作按鈕`。
* **下拉選單**（`7808:51844`）：`E1_interview_dropdown.png` — 裁切至橘色橫欄＋展開的下拉選單（改時間／取消面試／查看訊息）。標注徽章：`面試摘要列`、`下拉選單`。

**元素說明**（§1.3.6 規格表）：

| 元素 | 說明 |
| :--- | :--- |
| 面試摘要列 | 橘色橫欄：🗓 `面試：{日期} {時間}・面試地點：{地點}・聯絡人：{姓名}`；右側 `更多`/`更少` 切換展開 |
| 展開資訊 | `回覆期限`・`聯絡人`・`電話` |
| 操作按鈕 | `取消面試` / `改時間` |
| 下拉選單 | `改時間` / `取消面試` / `查看訊息` |

**裁切原則**：像素掃描找橘色背景（約 `#FFE9DC`–`#FFCDB8`）邊界；下至按鈕底部，不含系統訊息或對話泡泡；badge 疊於元素左上，不加左側空白 margin。
