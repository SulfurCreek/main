...
## Step 0（優先於反查）：先檢查正解是否已在現有2~5

**這是既有規則 `keep_strike` 的延伸**（見 [logic/01-plan-algorithm.md](../logic/01-plan-algorithm.md) 步驟4：
「correct_present = correct 已在某現有格？是＋有非保護錯誤格 → keep_strike」；`03-pitfalls.md` 亦有同一教訓：
「正解已存在不代表其他格都對」）。原本只在完整 `plan()` pipeline（`scripts/sync_md.py`）中實作，
本 skill 把它併入「職類1」簡化流程，補上這條**必須最先做**的檢查：

1. 看該筆現有2~5（不只現有1）是否已經有一格明顯符合職缺名稱的功能關鍵字。
2. **是** → 這是 `keep_strike` 型態：現有1 判錯要槓，`建議1` 直接填那個已存在的值即可
   （不必另外去反查 Description 才能下判斷——它已經是現有格之一，代表先前已被留在正解候選裡）。
3. **否**（現有2~5 都不含合理答案）→ 才進入下面的 Description 反查流程，找 tCodeDutyNM 全表中的候選葉。

範例：132466501「文件管制課-助理工程師」現有1=檔案資料管理人員（錯）、現有3=工程助理——
工程助理已經在現有格，直接 `建議1=工程助理` 即可，不需要先查 Description 才能確認（Description 反查是
用來佐證/在現有格都不像時才需要動用的重工具，不是每筆的必經步驟）。

## 反查判斷法（現有2~5 都無合理候選時才用）
