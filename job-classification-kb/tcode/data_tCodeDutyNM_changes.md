...
> 排除「不新增」「合併」及其他純討論註記。共 35 筆（5 新增、30 改名，含 3 筆中類層級改名）。
> 本檔為**建議清單**，尚未寫入 `tCodeDutyNM`／`TCode_Export.xlsx` 正式代碼表；代碼皆為暫擬，正式代碼以系統指派為準。

## 代碼結構規則（判斷新增代碼該掛在哪一層時要用到）

見 `01-schema.md` CodeType 階層；`tCodeDutyNM` 為三層表，CodeNo 規則：
...
判斷一筆「改名」是中類還是小類層級：CSV 中若 `1111小類代碼`／`1111小類名稱` 為空、只有 `1111中類名稱` 有值 → 該筆是**中類層級**改名，對應代碼末兩碼必為 `00`。

## 一、中類層級改名（3 筆）

| 中類 CodeNo | 現行中類名稱 | 建議新中類名稱 | 所屬大類 |
|---|---|---|---|
| 210200 | 化工實驗 | 化工／材料研發 | 生物科技／化學製藥 |
| 250500 | 教育師資 | 國教師資 | 學術研究／教育師資 |
| 260300 | 補習進修 | 補教師資 | 幼教才藝／補習進修 |

## 二、葉／小類層級新增（5 筆）

| 暫擬 CodeNo | 建議名稱 | 所屬中類 | 命名依據 | 說明 |
|---|---|---|---|---|
...
| 250514 | 華語教師 | 中文老師 | 教育師資→補教師資 | 連動：一併改移到「補教師資」中類 |
...
## 四、待進一步討論（原 CSV 標「不新增」但保留背景，僅供參考，非本次建議範圍）

- 290206 社區秘書、290203 其他保安服務工作等：CSV 原文標「不新增」但附完整背景說明，顯示廠商端仍有討論空間；本檔不予採用，僅記錄於原始 CSV，若日後重啟討論可回頭查對。
