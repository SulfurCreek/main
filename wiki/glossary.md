...
- **邀約訊息 / Interview Card**：聊天室內以卡片呈現的訊息類型，含 `詢問意願`／`面試邀約`／`錄取通知`／`婉拒（感謝函）`，依 `MailType` 顯示為對應泡泡。
...
- **意願狀態標籤**：求職者回覆後系統自動寫入、雙方都看得到的提示文字（如「施小君已回覆有意願面試」）。雙視角文案差異：廠商側用第三人稱主詞、求職者側用第二人稱（「你已…」）；有意願／同意＝綠色、無意願／婉拒＝灰色。
