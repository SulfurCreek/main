...
description: >
  ...
  疊加 badge／紅框／編號說明，都套用本 skill。取代舊版 `wiki/figma_rules.md`（該檔已收斂為一行指標）。
  觸發時機包含但不限於：「截圖」「badge」「標註」「框起來」「打編號」「Figma 截一張圖」「存到 repo」
  「畫流程圖／點擊流程」「這個按鈕框紅框」。
  核心規則：**標註一律用 HTML 絕對定位覆蓋（absolute positioning），不燒進圖片像素**——與
  `CLAUDE.md`「截圖標註」既有規範一致（不使用 Pillow 把 badge 畫進 PNG 的舊做法）。
---
...
> ⚠️ **政策異動（已生效，取代舊版 Step 0）**：本專案已決定**所有經 skill 處理、寫進 HackMD 文件的圖片，一律存 R2**，不再區分「已有 HackMD 網址就直接沿用」。理由：HackMD 自己的 `_uploads` 圖床沒有可靠的清單／刪除 API，圖片一多就沒辦法像 R2 那樣盤點、汰換、追蹤生命週期（這正是之前反覆發生「刪除舊圖、換新圖」時只能手動對答案的原因）。**即使圖片原本已經有 `hackmd.io/_uploads/...` 網址（使用者貼上來的、或文件裡既有的），只要這次要把它納入新產出的標註／流程圖，都要先下載、上傳到 R2、改用 R2 網址**，不能再嫌麻煩直接沿用。

**Step 0：判斷這張圖目前掛在哪個圖床**：
- **已經是 R2 網址**（`pub-xxxx.r2.dev/photo-skill/...`）：直接沿用，不用重存。
- **是 `hackmd.io/_uploads/...` 網址，或使用者純聊天貼圖／Figma fetch 短效網址／本地檔案**：一律走下面完整存檔流程，下載後上傳 R2，改用 R2 網址。
- 舊版「HackMD 網址就直接沿用、不用存 R2」的做法已停用；文件裡**既有、這次沒有動到**的 `hackmd.io/_uploads` 圖片不用主動回頭遷移，只有這次要處理／納入新產出的才要順手換成 R2。

若圖片**沒有**既有的 R2 網址（HackMD `_uploads` 網址、Figma MCP 剛截的圖、使用者上傳的本地檔案等），走以下存檔流程：
...
1. **取得原始圖檔**
   ...
   - **HackMD `_uploads` 既有網址來源**（使用者貼的 `hackmd.io/_uploads/xxx.png` 連結、或文件裡既有的裸圖）：`curl -sL` 直接下載即可，這類網址不需要登入憑證也不會過期，跟 Figma 短效連結不同，但一樣要下載落地才能上傳 R2。
...
5. **上傳後確認可達，而且要驗證「真正寫進文件的那個網址」，不是憑記憶／變數對的網址**（真實事故）：曾經發生上傳到 R2 時用了一組檔名（例如 `E1_132_operate_menu.png`），組 HTML 字串時卻手滑用了另一套簡稱（例如 `menu.png`）——上傳步驟自己 `curl -sI` 檢查的是**它以為存進去的檔名**，兩者對不上，檢查當下顯示 200（因為查的是真的存在的那個檔名），但實際 PATCH 進 HackMD 的 HTML 裡引用的是**另一個從未上傳過的檔名**，使用者打開文件全部顯示破圖。**修法：驗證永遠針對「最終要 PATCH 的那份 content 字串」做，不要針對「上傳腳本裡自己維護的檔名變數」做**——PATCH 前（或 PATCH 後 GET 回來），用正規表達式把 content 裡所有 `<img src="...">`（尤其 R2 網域）逐一取出，每個都 `curl -sI` 實測 200，而不是只驗證「我剛剛上傳的那幾個檔案」。兩者聽起來像同一件事，實際上是兩個不同的資料來源，會因為手誤（重複造了兩套命名）而不同步。
...
#### ⚠️⚠️ 硬性規定：沒有紅框的純圖片，也要用「外層 `div` 帶 `max-width`」包住，不能只靠 `<img>` 自己的 inline `max-width`（真實事故）

**症狀**：文件裡多張圖用同一套排版函式產生，其中有紅框標註的圖（`aspect-ratio` 容器包 `<img>`）尺寸都正常，唯獨**沒有紅框、單純展示的圖**（例如流程最後一步「顯示結果」這種純截圖）被撐得比其他圖大很多，跟同排版面明顯不成比例。

**根因**：純圖片沒有紅框需求，容易偷懶只寫 `<img src="..." style="width:100%; max-width:500px;">` 就結束，讓 `<img>` 標籤自己扛「限制顯示寬度」的責任、沒有額外包一層容器 `div`。HackMD 的樣式表對內文 `img` 有自己的規則（覆寫優先權可能蓋過 inline 樣式），一旦生效，`<img>` 自己的 inline `max-width` 就失效，圖片吃滿它所在 flex 欄位的可用寬度（可能撐到 700px 以上），比同一份文件其他有容器限制的圖大上一截。這跟本節上一條「`aspect-ratio` 容器」是同一個類型的問題：**任何限制圖片顯示尺寸的責任都不能只交給 `<img>` 自己的 inline style，一定要由外層容器頂住**。

**修法**：純圖片（不需要紅框／badge）一律也包一層固定 `max-width` 的 `div`，讓限制寬度的是容器而不是 `<img>` 本身：

```html
<div style="width:100%; max-width:500px; flex-shrink:0;">
<img src="<圖網址>" style="display:block; width:100%; border-radius:6px; border:1px solid #eee;">
</div>
```

不要寫成這樣（`<img>` 單獨扛限寬責任，容易被 host CSS 覆寫掉）：

```html
<img src="<圖網址>" style="display:block; width:100%; max-width:500px; border-radius:6px; border:1px solid #eee;">
```

**驗證**：跟 `aspect-ratio` 那條規則共用同一套「敵意模擬」測試（`.markdown-body img { max-width:100% !important; }`）——測試頁裡故意覆寫 `img` 的 `max-width`，渲染後用 `getBoundingClientRect().width` 讀出每張 `<img>` 實際寬度，逐一核對是否仍等於預期的顯示寬度，不是只看「有沒有溢出容器」（溢出檢查測不出「圖片變胖但沒超框」這種情況，必須直接比對寬度數值）。
...
### ⚠️ PATCH 前跟同文件既有段落並排比對——機械檢查測不出「比例不對／風格不一致」

**這類問題的共同點：所有機械檢查（溢出、URL 200、縮排、`aspect-ratio`）全數通過，問題只有肉眼並排看才會現形。** 兩次真實案例都是這樣被使用者抓到，而不是自檢清單抓到的：一次是同一份文件裡新章節的視覺風格明顯比舊章節花俏（見下方「不要自創樣式」的教訓）；一次是新區塊裡某張圖比同排其他圖明顯胖一截（見前面「純圖片限寬」事故）——兩次的根因都不同，但**都逃過了逐條打勾的自檢清單**，因為清單檢查的是「這個區塊本身有沒有符合規則」，不是「這個區塊跟隔壁區塊擺在一起協不協調」。

**修法**：PATCH 前，把新區塊跟同一份文件裡**既有的至少一個區塊**放進同一個敵意模擬測試頁（同一個 `#col` 容器、前後排列），渲染成一張截圖，自己 `Read` 出來做三件事：
1. **比例級距**：新區塊的圖片顯示寬度、卡片間距、留白，跟舊區塊是不是同一個量級——不是「新圖看起來也不算太離譜」，是「兩者放在一起完全融入、看不出接縫」。
2. **字級與顏色**：callout 字級、顏色、字重跟舊區塊逐項比對數值（不是憑印象），新舊不一致的地方要嘛統一、要嘛有明確理由（例如舊區塊本身就是待汰換的舊格式）。
3. **同一批圖片裡有沒有一張特別大或特別小**：這類問題單獨看那張圖不會發現「大」，只有跟同排其他圖片並排比較才看得出來（純圖片限寬那次事故就是這樣被抓到的）。

這一步不是「看整頁渲染有沒有明顯錯」（那是規則二第 4 點已經在做的），而是**刻意去比較兩個獨立區塊之間的一致性**，兩者要分開做，一個測「這塊對不對」，一個測「這塊跟別的塊合不合」。
...
- [ ] **每一張沒有紅框的純圖片，限寬責任是不是交給外層 `div` 而非 `<img>` 自己**（見規則二「基本結構」小節的純圖片規定）——敵意模擬測試時順手讀出每張 `<img>` 實際渲染寬度，跟預期顯示寬度逐一核對，不是只看有沒有溢出容器。
- [ ] **跟同一份文件既有段落並排渲染比對過**（見「PATCH 前跟同文件既有段落並排比對」小節）：比例級距、字級顏色、同排圖片有沒有一張特別大/小——這一步測的是「協不協調」，不是「這塊本身對不對」，兩者缺一不可。
...
- [ ] **圖片網址對**：這次處理／納入新產出的圖都已上傳 R2（含原本是 `hackmd.io/_uploads` 網址的），沒有殘留 hackmd.io 網址混在新產出裡；已上傳的都 `curl -sI` 回 200，且 HTML 裡沒有殘留 `file://` 本地路徑。
...
- **政策異動：不再區分「已有 HackMD 網址就沿用」，一律遷移到 R2**：舊版 Step 0 教的是「圖片已經有 `hackmd.io/_uploads/...` 網址就直接沿用，不用存 R2」，出發點是省一道存檔手續。但實際使用下來，這造成同一份文件裡圖片分散在兩個圖床（R2＋HackMD 自己的 `_uploads`），HackMD 那邊沒有清單／刪除 API，事後要盤點「這份文件到底引用了哪些圖」「舊圖有沒有真的清掉」只能手動對答案，跟規則一其他條目強調的「可追蹤、可驗證」精神互相矛盾。**修法：規則一 Step 0 已改為一律遷移到 R2**——即使圖片原本已經有 HackMD 網址，只要這次要處理／納入新產出，一樣下載後上傳 R2、改用 R2 網址；只有文件裡**這次完全沒動到**的既有裸圖不用主動回頭遷移。`hackmd.io/_uploads` 的圖不需要登入憑證即可 `curl` 下載（跟 Figma 短效連結不同），遷移動作很輕量，沒有理由不做。
- **圖片全部破圖＝上傳檔名跟 HTML 裡引用的檔名對不上（真實事故）**：曾經在同一個腳本裡，上傳到 R2 時用 `E1_132_operate_menu.png` 這種完整命名，組 HTML `<img src>` 字串時卻手滑複製到另一段用簡稱 `menu.png` 命名的舊程式碼，導致兩者不一致。上傳步驟自己 `curl -sI` 驗證時是驗證「它剛存的那個檔名」，回 200 一切正常；但 PATCH 進 HackMD 的 HTML 引用的是**另一個從未真正上傳過的檔名**，使用者打開文件看到的是全部圖片都破圖。這個 bug 在「上傳」跟「組 HTML」兩個步驟各自獨立維護檔名變數時特別容易發生，且本機任何驗證流程都測不出來（因為本機測試時我通常會刻意把 `src` 換成本地 `file://` 路徑，繞過了這個「R2 網址對不對得上」的問題）。**修法見規則一第 5 點與自檢清單**：最後一定要從**要送出的 content 字串本身**撈出所有圖片網址逐一 `curl` 驗證，不能只信「upload 腳本說它成功了」。
...
- **⚠️ 純圖片（無紅框）尺寸異常放大＝限寬責任只交給 `<img>` 自己的 inline `max-width`（真實事故）**：同一份文件裡多張圖用同一套排版函式產生，有紅框標註的圖都用「`position:relative` 容器＋`aspect-ratio`」包住，尺寸正常；唯獨一張純展示、不需要紅框的圖（流程最後一步的結果截圖）只寫了 `<img style="width:100%; max-width:500px;">`，沒有額外包容器。PATCH 上 HackMD 後，這張圖被撐得比其他圖明顯大一截——原因跟「紅框高度塌陷」是同一類問題：HackMD 的樣式表對內文 `img` 的覆寫規則蓋掉了 `<img>` 自己的 inline `max-width`，圖片吃滿它所在 flex 欄位的可用寬度。本機用寬 viewport 或乾淨頁面測試看不出來，因為那些環境沒有 HackMD 自己的 `img` 覆寫規則。**修法見規則二「基本結構」小節的「純圖片也要用外層 div 帶 max-width 包住」**：不管圖片上有沒有紅框，限制顯示寬度的永遠是外層容器，不是 `<img>` 自己的 style。敵意模擬測試時除了測「有沒有溢出」，也要讀出每張 `<img>` 的實際渲染寬度逐一核對數值，才抓得到「沒溢出但胖了」這種情況。
...
- **禁止事項**（沿用 `CLAUDE.md`「截圖標註」既有規則）：不使用或提及任何外部 AI 圖像生成模型重繪畫面；不得用 Pillow 把 badge 燒進像素後才存檔——那是舊流程，本 skill 生效後棄用。
