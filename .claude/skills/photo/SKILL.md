...
- [ ] **用 CSS 視窗裁切局部畫面時，badge 沒有被 `overflow:hidden` 的裁切容器切掉**：badge 是放在裁切容器外層的非遮罩 wrapper，不是放在裁切容器內部。
...
- **用 CSS 視窗裁切既有大圖時，badge 不能放在 `overflow:hidden` 的裁切容器內**：需要只顯示一張大截圖的局部（例如從一張完整畫面截圖裡只框出「訊息輸入區」那一小塊）又不想重新裁切存成新檔案時，可以用「外層固定尺寸＋`overflow:hidden`，內層 `<img>` 用負值 `top`/`left` 位移＋放大寬度」的技巧顯示局部——這不算產生新圖檔，符合規則一。但紅框／badge 若也放進這個 `overflow:hidden` 容器裡，只要座標算出來是負值或超出容器邊界（例如 badge 想放在框的「左上外側」，剛好卡在裁切窗最上緣之外），就會被容器的 `overflow:hidden` 直接切掉、完全不可見，而且**寬 viewport 或窄 viewport 都會出現同樣問題**（跟前一條「欄寬裁切」是不同成因，不能用同一招修）。修法：裁切容器只放 `<img>` 和「畫在圖片範圍內」的紅框（它們的座標本來就在 0–100% 之間，不會超界）；badge 一律移到**外層再包一層「無 `overflow:hidden`」的 `position:relative` 容器**，badge 對這層外層容器做絕對定位——外層沒有裁切遮罩，badge 即使座標算成負值（例如刻意放在框的正上方外側）也完整可見。PATCH 前務必用裁切放大檢查（見規則二「座標怎麼抓」最後一點）確認每個 badge 數字都完整可見，沒有被容器邊界切掉一半或完全消失。
