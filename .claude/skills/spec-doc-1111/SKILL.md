...
* Use Case: `{調用端＋執行動作＋受影響標的物}`
  * 若有多個調用情境（如跨系統回覆），改用子彈條列：`{第二個調用情境}`
...
- 標題下接 **User Story**（Persona 角度）與 **Use Case**（系統角度），以條列呈現、**內容整句用反引號包起來**（`User Story: \`身為…\``），不加其他修飾；Use Case 可有多條子彈（如跨系統情境分開列）。
...
**狀態說明統一收斂進表格欄位本身、避免另立小表格**：當一個欄位（如按鈕）在不同條件下有多種呈現，直接在該欄位用 `<ul><li>` 條列「條件：呈現結果」，不要在表格下方另開一個獨立的「OO 狀態」子表格或大段文字重述同一邏輯——欄位內容已經是唯一事實來源。
...
- [ ] 最上方有 `MD033`、`MD013` 兩行
- [ ] 標題 `# {代號} {功能名稱}` 下有 `* User Story:` + `* Use Case:`，內容整句包反引號（多個調用情境時 Use Case 用條列子彈）
