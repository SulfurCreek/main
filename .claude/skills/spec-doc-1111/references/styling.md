...
撰寫 `[REF] 職缺資料欄位表與代碼表` 這類資料庫欄位／代碼參考文件時（不是一般規格書，格式獨立）：

- **欄位表只留業務閱讀者用得到的欄位**：序號、欄位名稱、允許NULL、預設值、說明。不收 DB 儲存層細節（自動編號、主鍵、類型、佔用空間、長度、小數位數）——那些是 schema 匯出檔本身的資訊，不是規格閱讀者需要的。
- **代碼表門檻：10 筆為界**。代碼筆數 **未滿 10 筆**，直接寫進欄位表「說明」欄（格式：`欄位原說明(代碼表名稱：代碼1:名稱1／代碼2:名稱2／…)`，多值用全形 `／` 分隔，避免跟名稱裡本來就有的頓號、逗號混淆）；**10 筆以上**才獨立拉出一個 `## 代碼表` 底下的小節。同一組代碼被多個欄位共用時（如語言聽/說/讀/寫四個子欄位），只在第一個欄位完整列出，其餘寫「同 {欄位名}」。
- **小節內若要再分維度**（例如一個欄位其實對應兩組獨立代碼，如工作經驗的「年資」與「以上/限定/以下」），**用實際的 heading（比小節低一階，如 `####`）分開，不要用粗體字充當標題**——粗體不會進 TOC／不能被錨點連結，混用會造成文件目錄與實際結構對不上。
