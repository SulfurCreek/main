<!--markdownlint-disable MD033-->
<!--markdownlint-disable MD013-->

> 🧭 [← 職能框架首頁 / Home](../competency-framework.md)

# 完整職涯時間軸 / Career History

> F1–F11 記錄的是**現職（1111）**的職能；本頁補上 **2013–2022 的前段職涯**——
> 以 **B2C 行動產品**為主的近 6 年 PM 資歷，是 F12／F13／F14 的證據來源。
> 來源：本人前一版英文履歷（自撰）。

## 時間軸 / Timeline

| 期間 | 公司 | 職稱 | 產品 |
| :--- | :--- | :--- | :--- |
| 2022/08 – 至今 | **1111 人力銀行** | 產品企劃（求才系統主責）| 求才 B 端平台、公司頁、求才側 AI（見 F1–F11）|
| 2018/01 – 2022/03 | **尚凡國際創新科技 / Sunfun Info** | Project Manager | **JustDating**（App ＋ Web 版）、**KOOL 網紅人力銀行** |
| 2016/06 – 2018/01 | **思維特網路資訊 / Sweet Tech** | Associate Product Manager | **SweetRing** 交友 App |
| 2013/08 – 2014/02 | **台灣新蛋 / Newegg Taiwan** | Product Description Editor／Translator | Newegg.com 商品內容（英→中）|

**PM 年資累計約 10 年**（2016/06 起；扣除 Newegg 的編輯／翻譯職）；**總職涯橫跨 B2C 行動產品 → B2B 招募平台**。

> 📌 **舊履歷寫「5 years of hands-on experience」是 2022 年的版本，現已約 10 年**——
> 新履歷務必更新資歷年數與定位層級（Senior PM，而非中階）。

## 前段職涯的具體成績 / Prior-role achievements

### 尚凡國際 Sunfun（2018/01 – 2022/03，Project Manager）

- **從零初創多個服務平台**：從提案、建立 user story、wireframe 與 prototype，到用以展示架構與 user flow。
- **在嚴格時程下交付多次產品初版上線**，達成穩定且可用的 **MVP**。
- 以**成長模型（growth model）思維**調整專案計畫以符合利害關係人需求。
- **調整 IAP（應用內購）模型與價格點**、搭配階段性促銷，**提升營收與獲利**〔待補數據：成長 %〕。
- **以召回策略（call-back strategies）降低用戶流失**，策略源自對**中國行動應用市場案例**的研究〔待補數據：流失下降 %〕。
- **將第三方金流導入成熟產品**以觸及更多潛在銷售，並擔任**串接與營運期的單一窗口**。

### 思維特 Sweet Tech（2016/06 – 2018/01，Associate PM）

- 執行行動 App 的**日常營運**工作。
- **海外市場突破與社群經營**：與具 **美國／墨西哥／巴西／印尼／馬來西亞／日本／韓國** 背景成員的行銷團隊協作，
  完成**整個 App 的在地化**並透過 Facebook Pages 進入各市場。
- 向利害關係人**提案並通過多項功能、改版與優化**，包含改良使用者之間的 **Q&A 互動機制**以刺激更多配對與有意義的對話。
- 每日與**前端／後端工程、CRM、行銷、UI 設計**溝通協作。
- **監測用戶數據與趨勢**：iOS／Android 官方後台、第三方數據網站（如 **App Annie**）、用戶資料，據以產出洞察。

### 台灣新蛋 Newegg（2013/08 – 2014/02）

- 將 Newegg.com 的商品資訊、規格與描述**英翻中**。
- 改善既有上架內容的**可讀性與台灣在地化**。

## 產品已下架該怎麼寫 / Handling sunset products

部分服務（JustDating、KOOL、SweetRing）已下架或轉型。**這不影響履歷的有效性**：

- 履歷寫的是**你的角色與成果**，不是產品的現況——「主導 X 產品從 0 到上線」是已發生的事實。
- **不要**在履歷主動標註「已下架」；被問到就是中性事實：「該產品於 20XX 年結束營運。」
- 若擔心對方查不到產品，可在**作品集**用去識別化的方式描述（產品類型、規模、你的決策），而非依賴外部連結。
- 下架本身常是公司策略／市場因素，非個人績效問題；**準備一句話說明**即可（例：資源轉向主力產品線）。

## ⚠️ 待釐清的兩處不一致 / Two discrepancies to resolve

1. **畢業年份衝突**：舊履歷寫 **Tamkang University 2010–2014**，但本框架目前記為 **Class of 2013**（依你口述）。
   兩者不可能同時為真。**背景調查會查學歷**，請確認正確年份後統一
   （[學歷頁](education-certifications.md) 與新履歷必須一致）。
   參考：CSUS 交換為 2011–2012、TOEFL 於 2011/02 應考、Newegg 自 2013/08 任職。
2. **2014/02 – 2016/06 約 2 年 4 個月的空檔**：舊履歷未說明。這是履歷紅旗之一
   （見 `resume-craft` 的反面模式表）。請準備一句誠實說明（進修／兵役／家庭／轉職準備皆可），
   或在履歷上以年份（非月份）呈現以淡化。

## 舊履歷的品質問題（新版務必修正）

| 問題 | 舊履歷原文 | 修法 |
| :--- | :--- | :--- |
| **幾乎零量化** | "Grew sales and boosted profits"、"Successfully reduced user churn"、"Delivered numbers of product initial launch" | 補上 %／金額／用戶數；拿不到就用代理指標（產品數、市場數、團隊規模）|
| 模糊量詞 | "numbers of…"（多次／多項）| 改成確切數字：**3 個 0→1 ＋ 4 個既有產品** |
| 形容詞開頭 | "Successfully proposed…"、"Successfully introduced…" | 刪 `Successfully`，改強動詞（Spearheaded／Architected／Drove）|
| 職責而非成果 | "Carried out day-to-day mobile app operating routine tasks" | 改寫為帶結果的成就，或刪除 |
| 資歷過時 | "5 years of hands-on experience" | 更新為約 **10 年** |

---

**相關分頁 / Related**：[F12 C 端行動產品與 0→1](F12-consumer-mobile.md) ・ [F13 變現、定價與金流](F13-monetization-pricing.md) ・ [F14 國際化與在地化](F14-localization-intl.md) ・ [學歷・證照](education-certifications.md)
